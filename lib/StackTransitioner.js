...
export default class StackTransitioner extends Component {
  ...
  isFixed(element) {
    ...
    if (!children || !children.props[`${element}Component`]) return false; // current route has no headerComponent or footerComponent
  ...
  render() {
    const { stackViewStyle, renderHeader } = this.props;
    const { children, previousChildren, transition } = this.state;
    const Header = (children && children.props && children.props.headerComponent) || renderHeader;
    const Footer = children.props.footerComponent;
    const PreviousHeader = (previousChildren && previousChildren.props.headerComponent) || renderHeader;
